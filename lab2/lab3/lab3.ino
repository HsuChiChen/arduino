...
void loop() {
	...
	for(int i=0; i<Data_num; i++){
		...
		if(theta_x >= max_x) max_x = theta_x;
		else min_x = theta_x;
		
		if(theta_y >= max_y) max_y = theta_y;
		else min_y = theta_y;
		
		if(theta_z >= max_z) max_z = theta_z;
		else min_z = theta_z;
	}
	...
	if(delta_x>Threshold || delta_y>Threshold || delta_z>Threshold){
		...
		max_x = 0; min_x=0; delta_x = 0;
		max_y = 0; min_y=0; delta_y = 0;
		max_z = 0; min_z=0; delta_z = 0;
    delay(500);
	}
}
